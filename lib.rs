...
async fn add(left: i128, right: i128) -> i128 {
    let x = gstd::msg::send_for_reply_as::<_, Ops>(
        gstd::actor_id!("0xb982692d879d5d007fe85990944dcee6003e972d26ba5d29b8b2b0e886606731"),
        Ops::Add(left, right),
        0,
        0,
    )
...
async fn sub(left: i128, right: i128) -> i128 {
    let x = gstd::msg::send_for_reply_as::<_, Ops>(
        gstd::actor_id!("0x2628ef3dc3e9fc20841b6dbafc7aa0d07ee6a8c280408ecafec4ad31942c1d2d"),
        Ops::Sub(left, right),
        0,
        0,
    )
...
async fn div(left: i128, right: i128) -> i128 {
    let x = gstd::msg::send_for_reply_as::<_, Ops>(
        gstd::actor_id!("0xf6d81379de198461983d858c22e7e5df7f94ac92da7543619e254360ffd982ec"),
        Ops::Div(left, right),
        0,
        0,
    )
...
async fn pow(left: i128, right: u32) -> i128 {
    let x = gstd::msg::send_for_reply_as::<_, Ops>(
        gstd::actor_id!("0x2c8a8f52ea72325685eb429799f5a69e71f904a955efdd0e5f46dee352f1a0e3"),
        Ops::Pow(left, right),
        0,
        0,
    )
...
async fn modulus(left: i128, right: i128) -> i128 {
    let x = gstd::msg::send_for_reply_as::<_, Ops>(
        gstd::actor_id!("0xc27dd9384d11beafc4e99dfbbb4b2b8dc976895a47b2b02e11c0c6e8db7664bf"),
...
async fn neg(left: i128) -> i128 {
    let x = gstd::msg::send_for_reply_as::<_, Ops>(
        gstd::actor_id!("0x86ad9dd2c43e062ea50b366b7d80cb663df9d513f5c634fc05fc24fc314aed1a"),
        Ops::Neg(left),
        0,
        0,
    )
